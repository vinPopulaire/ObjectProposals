         ...
         )
{
    ...
    image<int> *seg = segment_image(r, g, b, sigma, K, min_size, &num_ccs, myedges); 
    ...
    int n = myedges.size();
    plhs[1] = mxCreateNumericMatrix(n, 3, mxSINGLE_CLASS, mxREAL);
    float *edges = (float*) mxGetData(plhs[1]);

    for (i=0; i<n; i++) {
        edges[ i + 0*n ] = (float) myedges[i].a;
        edges[ i + 1*n ] = (float) myedges[i].b;
        edges[ i + 2*n ] = (float) myedges[i].w;
    }
